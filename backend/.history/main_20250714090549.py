...
# --- Load ML model/scaler/columns once at startup ---
_model   = joblib.load("best_model.ubj")
...
def quality_metrics(
    ...
    return MetricsService(db).quality(
        selected_date,
...
def predict(input_data: PredictionInput, _: dict = Depends(get_current_user)):
    # prepare DataFrame
    df = pd.DataFrame([input_data.dict()])
    # ensure correct ordering / missing cols
