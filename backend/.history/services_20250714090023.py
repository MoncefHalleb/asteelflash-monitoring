...
class BoardService:
    ...
    def create(self, data: BoardCreate) -> BoardInfo:
        ...
        self.db.execute(
            text(f"INSERT INTO abb_dbo_board ({cols}) VALUES ({vals})"),
            fields
        )
    ...
    def update(self, board_id: int, data: BoardUpdate) -> BoardInfo:
        ...
        self.db.execute(
            text(f"UPDATE abb_dbo_board SET {sets} WHERE Id = :board_id"),
            fields
        )
    ...
    def import_csv(self, lines: List[str]) -> int:
        ...
        count = 0
        for row in reader:
            row["Valide"] = row.get("is_valid") in ("1","true","True")
            row["Date_Creation"] = datetime.utcnow()
            cols = ", ".join(row.keys())
            vals = ", ".join(f":{k}" for k in row)
            self.db.execute(
                text(f"INSERT INTO abb_dbo_board ({cols}) VALUES ({vals})"), row
            )
            count += 1
        return count
...
class TestService:
    ...
    def unique_by_metric(self, metric_column: str, metric_value: str, start: str, end: str) -> List[TestRecord]:
        if metric_column not in {"TypeTest","Result","Id_Machine","Id_Operateur","Id_Board","Num_Serie"}:
...
class MetricsService:
    ...
    def quality(self, d: date, start_t, end_t) -> QualityMetrics:
        start = f"{d} {start_t}"
        end   = f"{d} {end_t}"
        ...
        good = self.db.execute(text("""
            ...
            FROM abb_dbo_test 
            WHERE Result = 1 
              AND STR_TO_DATE(DateDebut,'%Y-%m-%d %H:%i:%s') 
              BETWEEN :start AND :end
        """), {"start": start, "end": end}).scalar() or 0
        ...
        defects = dict(self.db.execute(text("""
            SELECT Defaut, COUNT(*) 
            FROM abb_dbo_intervention 
            WHERE DateIntervention BETWEEN :start AND :end 
        ...
        ref_stats = self.db.execute(text("""
            ...
            WHERE STR_TO_DATE(t.DateDebut,'%Y-%m-%d %H:%i:%s') 
              BETWEEN :start AND :end
      ...
      MAX(t.Num_Serie)            AS num_serie      -- << add this alias
    FROM abb_dbo_board b
    JOIN abb_dbo_test t ON t.Id_Board = b.Id
    WHERE STR_TO_DATE(t.DateDebut,'%Y-%m-%d %H:%i:%s')
      BETWEEN :start AND :end
...
# build test_details with id_operateur cast to string
test_details = []
for row in tests:
    data = dict(row._mapping)
    op = data.get("id_operateur")
    data["id_operateur"] = str(int(op)) if op is not None else None
    test_details.append(TestRecord(**data))
