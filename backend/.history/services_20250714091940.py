...
class BoardService:
    ...
    def delete(self, board_id: int) -> None:
        self.db.execute(
            text("DELETE FROM abb_dbo_board WHERE Id = :board_id"),
            {"board_id": board_id}
        )
...
class TestService:
    ...
    def unique_by_metric(
        ...
        if metric_column not in {
            "TypeTest", "Result", "Id_Machine",
            "Id_Operateur", "Id_Board", "Num_Serie"
        }:
        ...
        sql = text(f"""
            WITH Ranked AS (
              SELECT
                t.*,
                ROW_NUMBER() OVER (
                    PARTITION BY t.Num_Serie
                    ORDER BY STR_TO_DATE(t.DateDebut, '%Y-%m-%d %H:%i:%s') DESC
                ) rn
              FROM abb_dbo_test t
